...
def main():
    ...
    size_mb = int(cfg.get("target_size_mb", 32))
    ...
    seed = ("FabricGovernor|FABRIC|TRAIN|v7").encode("utf-8")
    seed += fold_dataset(ds)
    if not seed:
        seed = ("FabricGovernor|FABRIC|TRAIN|EMPTY").encode("utf-8")
    seed = seed * 4096
    ...
    meta = {"agent":"FabricGovernor","size_mb":size_mb,"sha256":sha}
