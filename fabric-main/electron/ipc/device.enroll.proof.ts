...
export async function handle({ fp, pub, proof }: any) {
  ...
  const chalDir = path.join(SITE, 'enroll_challenges'); const pendDir = path.join(ROOT, 'devices', 'pending');
  ...
  const chalPath = path.join(chalDir, id + '.json'); if (!fs.existsSync(chalPath)) throw new Error('NO_CHALLENGE');
