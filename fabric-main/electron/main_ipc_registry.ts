...
/* Restored clean registry: ensures device.enroll.* handlers exist and are functions */
type IpcFn = (e: any, p: any) => any;

export const ipcHandlers: Record<string, IpcFn> = {
  "device.enroll.challenge": async (_e: any, { fp }: { fp: string }) => {
    const fs = require("fs");
  ...
  "device.enroll.proof": async (_e: any, { fp, pub, proof }: { fp: string; pub: string; proof: string }) => {
    const fs = require("fs");
    const path = require("path");
...
(() => {
  try {
    const reg: any = ipcHandlers;
    if (typeof reg["enroll.challenge"] === "function") reg["device.enroll.challenge"] = reg["enroll.challenge"];
    if (typeof reg["enroll.request"]   === "function") reg["device.enroll.proof"]     = reg["enroll.request"];

    try { (globalThis as any).ipcHandlers = reg; } catch {}
    try {
      if (typeof module !== "undefined" && (module as any).exports) {
        (module as any).exports.ipcHandlers = reg;
        (module as any).exports.default = reg;
      }
    } catch {}
  } catch {}
})();
