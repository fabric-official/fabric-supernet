...
int main() {
    ...
    if(!fabric::policy_check("WiFiAgent", "boot")) {
        std::cerr << "[policy] boot denied for WiFiAgent\n";
        return 1;
    }
    std::cout << "WiFiAgent boot OK" << std::endl;
    fabric::treasury_deposit("WiFiAgent", "boot", 0.01);
    return 0;
}
