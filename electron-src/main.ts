...
app.on("ready", async () => {
  ...
  ipcMain.handle("fabric:invoke", async (_e, payload: { channel: string; data?: any; capabilities?: string[] }) => {
    ...
    if (!channel || !ALLOWED_INVOKE.has(channel)) throw new Error("Blocked IPC channel");
    ...
    switch (channel) {
      ...
      case "plugin:get":
        return (G.__PLUGIN_REGISTRY__ ?? []).find((p: any) => p.id === data?.id) ?? null;
      case "env:version":
